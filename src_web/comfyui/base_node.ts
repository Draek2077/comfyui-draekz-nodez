...
import type {DraekzBaseServerNodeConstructor} from "typings/draekz.js";
...
import {DraekzHelpDialog} from "draekz/common/dialog.js";
import {defineProperty, moveArrayItem} from "draekz/common/shared_utils.js";
 ...
 */
export abstract class DraekzBaseNode extends LGraphNode {
  /**
  ...
  static override category = "draekz";
  static _category = "draekz"; // `category` seems to get reset by comfy, so reset to this after.
   ...
   * server nodes. DraekzBaseServerNode below overrides this with the expected value and we just
  ...
  private draekz_mode?: LGraphEventMode;

  /** An internal bool set when `onConstructed` is run. */
  private __constructed__ = false;
  /** The help dialog. */
  private helpDialog: DraekzHelpDialog | null = null;

  constructor(title = DraekzBaseNode.title, skipOnConstructedCall = true) {
    super(title);
    if (title == "__NEED_CLASS_TITLE__") {
      throw new Error("DraekzBaseNode needs overrides.");
    }
    ...
    setTimeout(() => {
      ...
      if (this.comfyClass == "__NEED_COMFY_CLASS__") {
        throw new Error("DraekzBaseNode needs a comfy class override.");
      }
    ...
    defineProperty(this, "mode", {
      get: () => {
        return this.draekz_mode;
      },
      set: (mode: LGraphEventMode) => {
        if (this.draekz_mode != mode) {
          const oldMode = this.draekz_mode;
          this.draekz_mode = mode;
  ...
  private checkAndRunOnConstructed() {
    if (!this.__constructed__) {
      this.onConstructed();
      const [n, v] = draekz.logger.logParts(
        LogLevel.DEV,
        `[DraekzBaseNode] Child class did not call onConstructed for "${this.type}.`,
      );
      console[n]?.(...v);
    }
  ...
  onConstructed() {
    ...
    draekz.invokeExtensionsAsync("nodeCreated", this);
    return this.__constructed__;
  }
  ...
  static setUp<T extends DraekzBaseNode>(...args: any[]) {
    // No-op.
  }
  ...
  showHelp() {
    ...
    if (help) {
      this.helpDialog = new DraekzHelpDialog(this, help).show();
...
/**
 * A virtual node. Right now, this is just a wrapper for DraekzBaseNode (which was the initial
 * base virtual node).
 */
export class DraekzBaseVirtualNode extends DraekzBaseNode {
  override isVirtualNode = true;

  constructor(title = DraekzBaseNode.title) {
    super(title, false);
  }

  static override setUp() {
    if (!this.type) {
      throw new Error(`Missing type for DraekzBaseVirtualNode: ${this.title}`);
    }
 ...
 */
export class DraekzBaseServerNode extends DraekzBaseNode {
  ...
  async setupFromServerNodeData() {
    ...
    for (const inputName in inputs) {
      ...
      // Also, it fixes https://github.com/comfyanonymous/ComfyUI/issues/1404 (for draekz nodes)
  ...
  static registerForOverride(
    ...
    draekzClass: DraekzBaseServerNodeConstructor,
  ) {
    ...
    OVERRIDDEN_SERVER_NODES.set(comfyClass, draekzClass);
    // Mark the draekzClass as `__registeredForOverride__` because ComfyUI will repeatedly call
    // this and certain setups will only want to setup once (like adding context menus, etc).
    if (!draekzClass.__registeredForOverride__) {
      draekzClass.__registeredForOverride__ = true;
      draekzClass.nodeType = comfyClass;
      draekzClass.nodeData = nodeData;
      draekzClass.onRegisteredForOverride(comfyClass, draekzClass);
    }
  }

  static onRegisteredForOverride(comfyClass: any, draekzClass: any) {
    // To be overridden
  }
}

/**
 * Keeps track of the comfyui-draekz-nodez nodes that come from the server (and want to be ComfyNodes) that
...
LiteGraph.registerNodeType = async function (nodeId: string, baseClass: any) {
  ...
  if (clazz !== baseClass) {
    ...
    console[n]?.(...v);
    // Note, we don't currently call our draekz.invokeExtensionsAsync w/ beforeRegisterNodeDef as
    // this runs right after that. However, this does mean that extensions cannot actually change
    // anything about overriden server draekz nodes in their beforeRegisterNodeDef (as when comfy
