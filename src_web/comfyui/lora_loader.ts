...
import {DraekzBaseServerNode} from "./base_node.js";
...
import {
  DraekzBaseHitAreas,
  DraekzBaseWidget,
  DraekzBetterButtonWidget,
  DraekzDividerWidget,
} from "./utils_widgets.js";
import {draekzApi} from "draekz/common/draekz_api.js";
import {showLoraChooser} from "./utils_menu.js";
import {moveArrayItem, removeArrayItem} from "draekz/common/shared_utils.js";
import {DraekzLoraInfoDialog} from "./dialog_info.js";
import {LORA_INFO_SERVICE} from "draekz/common/model_info_service.js";
...
/**
 * The Lora Loader is a super-simply Lora Loader node that can load multiple Loras at once
 * in an ultra-condensed node allowing fast toggling, and advanced strength setting.
 */
class DraekzLoraLoader extends DraekzBaseServerNode {
  ...
  private logger = draekz.newLogSession(`[Lora Loader]`);
  ...
  constructor(title = NODE_CLASS.title) {
    ...
    // since Lora Loader has dynamic widgets, this fails on ComfyUI's side. We can do so after
    // the fact but, unfortunately, we need to do it after a timeout since we don't have any
    // information at this point to be able to tell what data we need (like, even the node id, let
    // alone the actual data).
    if (draekz.loadingApiJson) {
      const fullApiJson = draekz.loadingApiJson;
      setTimeout(() => {
  ...
  override configure(
    ...
    for (const widgetValue of info.widgets_values || []) {
      if ((widgetValue as LoraLoaderWidgetValue)?.lora !== undefined) {
        const widget = this.addNewLoraWidget();
        widget.value = {...(widgetValue as LoraLoaderWidgetValue)};
      }
    }
  ...
  private addNewLoraWidget(lora?: string) {
    ...
    const widget = this.addCustomWidget(
      new LoraLoaderWidget("lora_" + this.loraWidgetsCounter),
    ) as LoraLoaderWidget;
  ...
  private addNonLoraWidgets() {
    moveArrayItem(
      this.widgets,
      this.addCustomWidget(new DraekzDividerWidget({marginTop: 4, marginBottom: 0, thickness: 0})),
      0,
    );
    moveArrayItem(this.widgets, this.addCustomWidget(new LoraLoaderHeaderWidget()), 1);

    this.widgetButtonSpacer = this.addCustomWidget(
      new DraekzDividerWidget({marginTop: 4, marginBottom: 0, thickness: 0}),
    ) as DraekzDividerWidget;

    this.addCustomWidget(
      new DraekzBetterButtonWidget(
        "Add Lora",
        (event: CanvasMouseEvent, pos: Vector2, node: TLGraphNode) => {
          draekzApi.getLoras().then((lorasDetails) => {
  ...
  override getSlotMenuOptions(slot: IFoundSlot) {
    ...
    if (slot?.widget?.name?.startsWith("lora_")) {
      const widget = slot.widget as LoraLoaderWidget;
      ...
      new LiteGraph.ContextMenu(menuItems, {
        title: "LORA WIDGET",
        event: draekz.lastCanvasMouseEvent!,
      });
  ...
  static override setUp(comfyClass: typeof LGraphNode, nodeData: ComfyNodeDef) {
    DraekzBaseServerNode.registerForOverride(comfyClass, nodeData, NODE_CLASS);
  }
  ...
  override getHelp() {
    return `
      <p>
        The ${this.type!.replace("(draekz)", "")} is a powerful node that condenses 100s of pixels
...
/**
 * The LoraLoaderHeaderWidget that renders a toggle all switch, as well as some title info
 * (more necessary for the double model & clip strengths to label them).
 */
class LoraLoaderHeaderWidget extends DraekzBaseWidget<{type: string}> {
  override value = {type: "LoraLoaderHeaderWidget"};
  override readonly type = "custom";

  protected override hitAreas: DraekzBaseHitAreas<"toggle"> = {
    toggle: {bounds: [0, 0] as Vector2, onDown: this.onToggleDown},
  };

  private showModelAndClip: boolean | null = null;

  constructor(name: string = "LoraLoaderHeaderWidget") {
    super(name);
  }

  draw(
    ctx: CanvasRenderingContext2D,
    node: DraekzLoraLoader,
    w: number,
    posY: number,
    height: number,
  ) {
  ...
  onToggleDown(event: CanvasMouseEvent, pos: Vector2, node: TLGraphNode) {
    (node as DraekzLoraLoader).toggleAllLoras();
    this.cancelMouseDown();
    return true;
  }
}

const DEFAULT_LORA_WIDGET_DATA: LoraLoaderWidgetValue = {
  on: true,
 ...
 */
class LoraLoaderWidget extends DraekzBaseWidget<LoraLoaderWidgetValue> {
  override readonly type = "custom";

  /** Whether the strength has changed with mouse move (to cancel mouse up). */
  private haveMouseMovedStrength = false;
  private loraInfoPromise: Promise<DraekzModelInfo | null> | null = null;
  private loraInfo: DraekzModelInfo | null = null;

  private showModelAndClip: boolean | null = null;

  protected override hitAreas: DraekzBaseHitAreas<
    | "toggle"
    | "lora"
    // | "info"
    | "strengthDec"
  ...
  override serializeValue(
    node: TLGraphNode,
    index: number,
  ): LoraLoaderWidgetValue | Promise<LoraLoaderWidgetValue> {
    const v = {...this.value};
  ...
  showLoraInfoDialog() {
    ...
    const infoDialog = new DraekzLoraInfoDialog(this.value.lora).show();
...
const NODE_CLASS = DraekzLoraLoader;

/** Register the node. */
app.registerExtension({
  name: "draekz.LoraLoader",
