 ...
 * dev console like `draekz_tests.TestDescribeLabel()`. The output is in the test itself.
 */
import { wait } from "draekz/common/shared_utils.js";
...
window.draekz_tests = window.draekz_tests || {};
...
export function describe(label: string, fn: Function) {
  if (!label.startsWith('Test')) {
   ...
   window.draekz_tests[label] = async () => {
    await describeRun(label, fn);
  };
  return window.draekz_tests[label];
}
