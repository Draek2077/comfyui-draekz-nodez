...
import type {Constructor} from "typings/draekz";

import {app} from "scripts/app.js";
import {api} from "scripts/api.js";
import {Resolver, getResolver, wait} from "draekz/common/shared_utils.js";
...
export function addMenuItemOnExtraMenuOptions(
  ...
  let idx = menuOptions
    .slice()
    .reverse()
    .findIndex((option) => (option as any)?.isDraekz);
  if (idx == -1) {
  ...
  menuOptions.splice(idx, 0, {
    ...
    isDraekz: true, // Mark it, so we can find it.
    callback: (
    ...
    node,
    {
      ...
      callback: (node) => {
        if ((node as any).showHelp) {
          ...
          new DraekzHelpDialog(node, content).show();
        }
      },
    },
    menuOptions,
