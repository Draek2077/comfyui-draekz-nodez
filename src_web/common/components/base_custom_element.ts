import {$el, getActionEls} from "draekz/common/utils_dom.js";
...
function getCommonPath(name: string, extension: string) {
  return `draekz/common/components/${name.replace("draekz-", "").replace(/\-/g, "_")}.${extension}`;
}

/**
 * Fetches the stylesheet for the component, matched by the element name (minus the "draekz-"
 * prefix).
 */
async function getStyleSheet(name: string, markupOrPath: string) {
  ...
  if (!CSS_STYLE_SHEETS.has(name)) {
    try {
      ...
      // alert("Error loading draekz custom component css.");
    }
  }
...
async function getTemplateMarkup(name: string, markupOrPath: string) {
  ...
  if (!HTML_TEMPLATE_FILES.has(name)) {
    try {
      ...
      // alert("Error loading draekz custom component markup.");
    }
  }
 ...
 */
export abstract class DraekzCustomElement extends HTMLElement {
  static readonly NAME: `draekz-${string}` = "draekz-override";
  static readonly USE_SHADOW: boolean = true;
  static readonly TEMPLATES: string = "";
  static readonly CSS: string = "";

  static create<T extends DraekzCustomElement>(): T {
    if (this.NAME === "draekz-override") {
  ...
  protected ctor = this.constructor as typeof DraekzCustomElement;
  ...
  async connectedCallback() {
    ...
    if (!this.hasBeenConnected) {
      ...
      } else if (this.templates.has(elementName.replace("draekz-", ""))) {
        template = this.templates.get(elementName.replace("draekz-", ""));
      }
      if (template) {
