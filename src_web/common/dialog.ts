...
export type DraekzDialogOptions = {
...
export class DraekzDialog extends EventTarget {
  ...
  options: DraekzDialogOptions;

  constructor(options: DraekzDialogOptions) {
    super();
...
export class DraekzHelpDialog extends DraekzDialog {
  constructor(
    ...
    content: string,
    opts: Partial<DraekzDialogOptions> = {},
  ) {
    const title = (node.type || node.title || "").replace(
      /\s*\(draekz\).*/,
      " <small>by draekz</small>",
    );
