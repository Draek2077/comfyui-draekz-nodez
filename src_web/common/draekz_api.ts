...
class DraekzApi {
  ...
  setBaseUrl(baseUrlArg?: string) {
    ...
    if (baseUrlArg) {
      baseUrl = baseUrlArg;
    } else if (window.location.pathname.includes("/draekz/")) {
      // Try to find how many relatives paths we need to go back to hit ./draekz/api
      const parts = window.location.pathname.split("/draekz/")[1]?.split("/");
      if (parts && parts.length) {
        baseUrl = parts.map(() => "../").join("") + "draekz/api";
      }
    }
    this.baseUrl = baseUrl || "./draekz/api";

    // Calculate the comfyUI api base path by checkin gif we're on an draekz independant page (as
    // we'll always use '/draekz/' prefix) and, if so, assume the path before `/draekz/` is the
    // base path. If we're not, then just use the same pathname logic as the ComfyUI api.js uses.
    const comfyBasePathname = location.pathname.includes("/draekz/")
      ? location.pathname.split("draekz/")[0]!
      : location.pathname;
  ...
  async postJson(route: string, json: any) {
    ...
    return await draekzApi.fetchJson(route, {method: "POST", body});
  }
  ...
  async getModelsInfo(options: GetModelsInfoOptions): Promise<DraekzModelInfo[]> {
    ...
    return (await this.fetchApiJsonOrNull<DraekzModelInfo[]>(path)) || [];
  }
  ...
  async refreshModelsInfo(options: ModelsOptions) {
    ...
    const infos = await this.fetchApiJsonOrNull<DraekzModelInfo[]>(path);
    return infos;
  }
  ...
  async clearModelsInfo(options: ModelsOptions) {
    ...
    await this.fetchApiJsonOrNull<DraekzModelInfo[]>(path);
    return;
  }
  ...
  async saveModelInfo(
    type: ModelInfoType,
    file: string,
    data: Partial<DraekzModelInfo>,
  ): Promise<DraekzModelInfo | null> {
    const body = new FormData();
    body.append("json", JSON.stringify(data));
    return await this.fetchApiJsonOrNull<DraekzModelInfo>(
...
export const draekzApi = new DraekzApi();
