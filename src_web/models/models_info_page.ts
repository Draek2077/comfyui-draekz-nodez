...
import {DraekzModelInfoCard} from "./components/model-info-card.js";
...
export class ModelsInfoPage {
  ...
  async init() {
    ...
    const loras = await draekzApi.getLorasInfo({light: true});
    console.log(loras);

    const baseModels = new Set<string>();

    for (const lora of loras) {
      const el = DraekzModelInfoCard.create<DraekzModelInfoCard>();
  ...
  filter() {
    ...
    const els = queryAll<DraekzModelInfoCard>(DraekzModelInfoCard.NAME);
    const shouldHide = (el: DraekzModelInfoCard) => {
