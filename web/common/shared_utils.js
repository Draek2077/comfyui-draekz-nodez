...
export function defineProperty(instance, property, desc) {
    ...
    if ((existingDesc === null || existingDesc === void 0 ? void 0 : existingDesc.configurable) === false) {
        throw new Error(`Error: comfyui-draekz-nodez cannot define un-configurable property "${property}"`);
    }
...
export class Broadcaster extends EventTarget {
    ...
    openWindowAndWaitForMessage(draekzPath, windowName) {
        const id = this.getId();
        this.queue[id] = getResolver();
        const win = window.open(`/draekz/${draekzPath}#broadcastLoadMsgId=${id}`, windowName);
    ...
    onMessage(e) {
        ...
        if (queueItem) {
            ...
            this.dispatchEvent(new CustomEvent("draekz-broadcast-message", {
                detail: Object.assign({ replyTo: (_b = e.data) === null || _b === void 0 ? void 0 : _b.id }, e.data),
            }));
        }
    }
    addMessageListener(callback, options) {
        return super.addEventListener("draekz-broadcast-message", callback, options);
    }
}
