...
import { iconGear, iconNode, iconReplace, iconStarFilled, logoDraekz, } from "../../draekz/common/media/svgs.js";
...
const INVOKE_EXTENSIONS_BLOCKLIST = [
    {
        ...
        reason: "Major conflict with comfyui-draekz-nodez nodes' inputs causing instability and " +
...
class Draekz extends EventTarget {
    constructor() {
        ...
        this.logger = new LogSession("[draekz]");
        ...
        if (CONFIG_SERVICE.getConfigValue("debug.keys_down.enabled")) {
            const elDebugKeydowns = createElement("div.draekz-debug-keydowns", {
    ...
    async invokeExtensionsAsync(method, ...args) {
        ...
        if (CONFIG_SERVICE.getConfigValue("features.invoke_extensions_async.node_created") === false) {
            const [m, a] = this.logParts(LogLevel.INFO, `Skipping invokeExtensionsAsync for applicable comfyui-draekz-nodez nodes`);
        ...
        return await Promise.all(comfyapp.extensions.map(async (ext) => {
            ...
            if (ext === null || ext === void 0 ? void 0 : ext[method]) {
                try {
                    ...
                    if (blocked) {
                        const [n, v] = this.logger.logPartsOnceForTime(LogLevel.WARN, 5000, `Blocked extension '${ext.name}' method '${method}' for draekz-nodez because: ${blocked.reason}`);
                        (_a = console[n]) === null || _a === void 0 ? void 0 : _a.call(console, ...v);
                        return Promise.resolve();
                    }
                    ...
                    const [n, v] = this.logParts(LogLevel.ERROR, `Error calling extension '${ext.name}' method '${method}' for draekz-node.`, { error }, { extension: ext }, { args });
    ...
    getDraekzIContextMenuValues() {
        ...
        return [
            ...
            },
            {
                ...
                submenu: {
                    ...
                    callback: (value, options, event) => {
                        const node = LiteGraph.createNode(addDraekz(value));
                        if (node) {
                            node.pos = [
            ...
            },
            {
                ...
                className: "draekz-contextmenu-item",
                callback: (...args) => {
                    const msg = `Convert ${rerouteLabel} ComfyUI Reroutes to Reroute (draekz) nodes? \n` +
    ...
    initializeComfyUIHooks() {
        ...
        api.queuePrompt = async function (index, prompt) {
            var _a;
            if (((_a = draekz.queueNodeIds) === null || _a === void 0 ? void 0 : _a.length) && prompt.output) {
        ...
        app.loadGraphData = function (graph) {
            ...
            setTimeout(() => {
                ...
                if (fixBadLinksResult.hasBadLinks) {
                    const [n, v] = draekz.logParts(LogLevel.WARN, `The workflow you've loaded has corrupt linking data. Open ${new URL(location.href).origin}/draekz/link_fixer to try to fix.`);
...
window.draekz = draekz;
app.registerExtension({
    name: "Comfy.DraekzComfy",
    aboutPageBadges: [
        {
