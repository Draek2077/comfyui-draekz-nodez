...
import { DraekzBaseWidget, DraekzBetterButtonWidget, DraekzDividerWidget, } from "./utils_widgets.js";
...
class DraekzLoraLoader extends DraekzBaseServerNode {
    constructor(title = NODE_CLASS.title) {
        ...
        this.logger = draekz.newLogSession(`[Lora Stack]`);
    ...
    addNewLoraWidget(lora) {
        ...
        const widget = this.addCustomWidget(new LoraLoaderWidget("lora_" + this.loraWidgetsCounter));
        if (lora)
            widget.setLora(lora);
    ...
    addNonLoraWidgets() {
        moveArrayItem(this.widgets, this.addCustomWidget(new DraekzDividerWidget({ marginTop: 4, marginBottom: 0, thickness: 0 })), 0);
        moveArrayItem(this.widgets, this.addCustomWidget(new LoraLoaderHeaderWidget()), 1);
        this.widgetButtonSpacer = this.addCustomWidget(new DraekzDividerWidget({ marginTop: 4, marginBottom: 0, thickness: 0 }));
        this.addCustomWidget(new DraekzBetterButtonWidget("Add Lora", (event, pos, node) => {
...
DraekzLoraLoader.title = NodeTypesString.POWER_LORA_LOADER;
DraekzLoraLoader.type = NodeTypesString.POWER_LORA_LOADER;
DraekzLoraLoader.comfyClass = NodeTypesString.POWER_LORA_LOADER;
DraekzLoraLoader[_a] = {
    type: "combo",
    values: [PROP_VALUE_SHOW_STRENGTHS_SINGLE, PROP_VALUE_SHOW_STRENGTHS_SEPARATE],
};
class LoraLoaderHeaderWidget extends DraekzBaseWidget {
    constructor(name = "LoraLoaderHeaderWidget") {
        super(name);
        this.value = { type: "LoraLoaderHeaderWidget" };
        this.type = "custom";
        this.hitAreas = {
...
class LoraLoaderWidget extends DraekzBaseWidget {
    constructor(name) {
        super(name);
        this.type = "custom";
