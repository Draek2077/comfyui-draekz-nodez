...
export class DraekzBaseNode extends LGraphNode {
    ...
    checkAndRunOnConstructed() {
        ...
        if (!this.__constructed__) {
            this.onConstructed();
            const [n, v] = draekz.logger.logParts(LogLevel.DEV, `[DraekzBaseNode] Child class did not call onConstructed for "${this.type}.`);
...
DraekzBaseNode.exposedActions = [];
DraekzBaseNode.title = "__NEED_CLASS_TITLE__";
DraekzBaseNode.type = "__NEED_CLASS_TYPE__";
DraekzBaseNode.category = "draekz";
DraekzBaseNode._category = "draekz";
...
export class DraekzBaseServerNode extends DraekzBaseNode {
    ...
    static registerForOverride(comfyClass, nodeData, draekzClass) {
    ...
    static onRegisteredForOverride(comfyClass, draekzClass) {
    }
}
DraekzBaseServerNode.nodeType = null;
DraekzBaseServerNode.nodeData = null;
DraekzBaseServerNode.__registeredForOverride__ = false;
...
LiteGraph.registerNodeType = async function (nodeId, baseClass) {
    ...
    if (clazz !== baseClass) {
        ...
        const [n, v] = draekz.logger.logParts(LogLevel.DEBUG, `${nodeId}: replacing default ComfyNode implementation with custom ${classLabel} class.`);
        (_a = console[n]) === null || _a === void 0 ? void 0 : _a.call(console, ...v);
    }
