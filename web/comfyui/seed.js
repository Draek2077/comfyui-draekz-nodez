...
class DraekzSeed extends DraekzBaseServerNode {
    constructor(title = DraekzSeed.title) {
        super(title);
        this.serialize_widgets = true;
        this.logger = draekz.newLogSession(`[Seed]`);
        ...
        draekz.addEventListener("comfy-api-queue-prompt-before", this.handleApiHijackingBound);
    }
    onRemoved() {
        draekz.addEventListener("comfy-api-queue-prompt-before", this.handleApiHijackingBound);
    }
    ...
    static setUp(comfyClass, nodeData) {
        DraekzBaseServerNode.registerForOverride(comfyClass, nodeData, DraekzSeed);
    }
    static onRegisteredForOverride(comfyClass, ctxClass) {
        addConnectionLayoutSupport(DraekzSeed, app, [
            ["Left", "Right"],
            ["Right", "Left"],
        ]);
        setTimeout(() => {
            DraekzSeed.category = comfyClass.category;
        });
    }
}
DraekzSeed.title = NodeTypesString.SEED;
DraekzSeed.type = NodeTypesString.SEED;
DraekzSeed.comfyClass = NodeTypesString.SEED;
DraekzSeed.exposedActions = ["Randomize Each Time", "Use Last Queued Seed"];
app.registerExtension({
    name: "draekz.Seed",
    async beforeRegisterNodeDef(nodeType, nodeData) {
        if (nodeData.name === DraekzSeed.type) {
            DraekzSeed.setUp(nodeType, nodeData);
        }
    },
});
