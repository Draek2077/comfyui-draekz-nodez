...
def get_config_value(key):
  return get_dict_value(DRAEKZ_CONFIG, key)
...
def set_user_config(data: dict):
  ...
  for key, value in data.items():
    if dict_has_key(DEFAULT_CONFIG, key):
      ...
      set_dict_value(DRAEKZ_CONFIG, key, value)
      count += 1
  if count > 0:
    write_user_config()


def get_draekz_default_config():
  """ Gets the default configuration."""
  return load_json_file(DEFAULT_CONFIG_FILE, default={})


def get_draekz_user_config():
...
DEFAULT_CONFIG_FILE = os.path.join(THIS_DIR, '..', 'draekz_config.json.default')
USER_CONFIG_FILE = os.path.join(THIS_DIR, '..', 'draekz_config.json')
...
def refresh_config():
  ...
  global DEFAULT_CONFIG, USER_CONFIG, DRAEKZ_CONFIG
  DEFAULT_CONFIG = get_draekz_default_config()
  USER_CONFIG = get_draekz_user_config()
  ...
  DRAEKZ_CONFIG = {"version": VERSION} | extend_config(DEFAULT_CONFIG, USER_CONFIG)

  if "unreleased" in USER_CONFIG and "unreleased" not in DRAEKZ_CONFIG:
    DRAEKZ_CONFIG["unreleased"] = USER_CONFIG["unreleased"]

  if "debug" in USER_CONFIG and "debug" not in DRAEKZ_CONFIG:
    DRAEKZ_CONFIG["debug"] = USER_CONFIG["debug"]


def get_config():
  """Returns the congfig."""
  return DRAEKZ_CONFIG
