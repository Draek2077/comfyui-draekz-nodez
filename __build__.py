...
from py.config import DRAEKZ_CONFIG
...
def build(without_tests = True, fix = False):
  ...
  # Handle the common directories. Because ComfyUI loads under /extensions/comfyui-draekz-nodez we can't
  # easily share sources outside of the `DIR_WEB_COMFYUI` _and_ allow typescript to resolve them in
  # src view, so we set the path in the tsconfig to map an import of "draekz/common" to the
  # "src_web/common" directory, but then need to rewrite the comfyui JS files to load from
  # "../../draekz/common" (which we map correctly in draekz_server.py).
  ...
  for file in js_files:
    ...
    if rel_path.startswith('comfyui'):
      filedata = re.sub(r'(from\s+["\'])draekz/', f'\\1{"../" * (num + 1)}draekz/', filedata)
      filedata = re.sub(r'(from\s+["\'])scripts/', f'\\1{"../" * (num + 1)}scripts/', filedata)
      # Dynamic Imports
      filedata = re.sub(r'(import\(["\'])draekz/', f'\\1{"../" * (num + 1)}draekz/', filedata)
    else:
      filedata = re.sub(r'(from\s+["\'])draekz/', f'\\1{"../" * num}', filedata)
      filedata = re.sub(r'(from\s+["\'])scripts/', f'\\1{"../" * (num + 1)}scripts/', filedata)
      # Dynamic Imports
      filedata = re.sub(r'(import\(["\'])draekz/', f'\\1{"../" * num}', filedata)
